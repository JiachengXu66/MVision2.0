...
/**
 * @description Creates new report in the database. Follows up by adding the required classes
 *              to the reportclass map. 
 */
router.post('/create', async(req,res,next)=>{
    ...
    const reportID = result[0].insert_reports
    const params_map = [reportID,JSON.stringify(_classes)];
...
/**
 * @description Gets latest report data given a limit of items per page and the current page number 
 *              (works via the report index)
 */
...
/**
 * @description Gets report data for a specific report ID
 */
...
/**
 * @description Gets an aggregation of classes for a specific task ID from the reportclass map
 */
...
/**
 * @description Gets the graph ID related to the current report
 */
...
/**
 * @description Gets data within the given date range and above a certain confidence threshold.
 *              Metric is a legacy parameter and needs to be removed to accomodate restructure 
 *              of dataTotalEntry table
 */
