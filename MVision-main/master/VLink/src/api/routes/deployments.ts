...
/**
 * @description Attempts to create a deployment. 
 *              If the deployment is supposed be active as of the current date it collates required
 *              information (device details and the node ip address) and calls the Posts to the node server.
 */
router.post('/create', async(req,res,next)=>{
    ...
    try{
        if (_start_date <= _creation_date && _creation_date < _expiry_date) {
            ...
            const deviceParams = [JSON.stringify([_device_id])];
            const deviceResult = await executeF("vision_data","get_devices_details",deviceParams);
            const device_name = deviceResult[0].get_devices_details.devices[0].device_name;
            const addressParams = [_node_id];
            const addressResults = await executeF("vision_data","get_addr_from_node", addressParams);
            const address = addressResults[0].get_addr_from_node.node_address;
            const ip = isIPv6(address) && address.includes('::ffff:') ? address.split('::ffff:')[1] : address;
            const initialiseDeploymentURL = `http://${ip}:2500/node/deployments/initialise`;
            ...
            try{
                const deploymentResults = await axios.post(initialiseDeploymentURL, deploymentInfo, {
                    headers: {
                ...
                res.status(200).json(deploymentResults);
            }
            catch(e){
                const duration = Math.round((Date.now() - startTime) / 1000);
        ...
        {
            const insertResponse = await executeF("vision_data","insert_deployments", params)
...
/**
 * @description Switches the status of the deployment based on whether it is active or disabled. 
                To switch status of an active or disabled deployment, its node must be live to change begin/stop inference.
                If disabling, calls the node server with stop endpoint.
                If activating, calls the node server with initialise endpoint.
 */
router.post('/switch/status', async(req,res,next)=>{
    ...
    if(_status_value == 'Active' || _status_value == 'New'){
        const deviceParams = [JSON.stringify([_device_id])];
        const deviceResult = await executeF("vision_data","get_devices_details",deviceParams);
        const deviceName = deviceResult[0].get_devices_details.devices[0].device_name;
        const addressParams = [_node_id];
        const addressResults = await executeF("vision_data","get_addr_from_node", addressParams);
        const address = addressResults[0].get_addr_from_node.node_address;
        const ip = isIPv6(address) && address.includes('::ffff:') ? address.split('::ffff:')[1] : address;
        const removeDeploymentURL = `http://${ip}:2500/node/deployments/stop`;
        const deploymentInfo = {
            "deployment_information": [
                {
                    "deployment_id": _deployment_id,
        ...
        try{
            const initialiseResults = await axios.post(removeDeploymentURL, deploymentInfo, {
                headers: {
                    'Content-Type': 'application/json',
                }
            })
            const updateParams = [_deployment_id, "Disabled"]
            const updateResults = await executeF("vision_data","update_deployment_status",updateParams)
            const duration = Math.round((Date.now() - startTime) / 1000);
    ...
    else if(_status_value == 'Disabled')
    {
        ...
        try{
            const initialiseResults = await axios.post(initialiseDeploymentURL, deployment_information, {
                headers: {
                    'Content-Type': 'application/json',
                }
            })
            const updateParams = [_deployment_id, "Active"]
            const updateResults = await executeF("vision_data","update_deployment_status",updateParams)
            const duration = Math.round((Date.now() - startTime) / 1000);
...
/**
 * @description Gets latest deployment data given a limit of items per page and the current page number 
 *              (works via the deployment index)
 */
...
/**
 * @description Gets deployment data for a specific deployment ID
 */
