...
router.get('/id/:node_id/devices/connected', async (req, res, next) => {
    ...
    const device_result = await executeF("vision_data", "get_node_devices", params);
    const device_ids = device_result[0].get_node_devices.devices;
    params = [JSON.stringify(device_ids)];
